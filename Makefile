...
init: ## Create schema and an empty warehouse
	$(PY) -m llmmeta.cli init --db $(DB)

registry: ## Import the curated 115-source registry
	$(PY) -m llmmeta.cli registry import research/leaderboard_census.csv --db $(DB)

discover: ## Snapshot the HF official-benchmark census
	$(PY) -m llmmeta.cli discover hf-official --db $(DB) --output data/raw/discovery/hf_official_$(DATE).json

ingest: ## Fetch + ingest all live adapters (backbone + Tier-B/C)
	$(PY) -m llmmeta.cli ingest --source openevals --db $(DB)
	$(PY) -m llmmeta.cli ingest --source openrouter --db $(DB)
	$(PY) -m llmmeta.cli ingest --source lmarena --db $(DB)
	$(PY) -m llmmeta.cli ingest --source aider_polyglot --db $(DB)
	$(PY) -m llmmeta.cli ingest --source artificial_analysis --db $(DB)
	$(PY) -m llmmeta.cli ingest --source vendor_claims --db $(DB)
	$(PY) -m llmmeta.cli ingest --source hf_official --db $(DB)

prices: ## Refresh provider pricing records
	$(PY) -m llmmeta.cli prices refresh --db $(DB) --as-of $(DATE)

normalize: ## Compute benchmark-local normalized scores
	$(PY) -m llmmeta.cli normalize --db $(DB) --method tie-aware-ecdf-v1

recommend: ## Run a profile end-to-end
	$(PY) -m llmmeta.cli recommend --profile $(PROFILE) --db $(DB) --output-dir outputs/$(basename $(notdir $(PROFILE)))

recommend-all: ## Run every profile in profiles/
	@for p in profiles/*.toml; do \
	  name=$$(basename $$p .toml); \
	  echo "→ $$name"; \
	  $(PY) -m llmmeta.cli recommend --profile $$p --db $(DB) --output-dir outputs/$$name >/dev/null; \
	done

export: ## Export catalog CSVs
	$(PY) -m llmmeta.cli export catalogs --db $(DB) --output-dir outputs/catalogs

test: ## Run the test suite
	$(PY) -m pytest -q

check: ## Database integrity checks
	$(PY) -m llmmeta.cli check --db $(DB)
